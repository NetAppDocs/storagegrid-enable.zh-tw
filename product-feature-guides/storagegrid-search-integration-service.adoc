...
平台服務是由S3租戶透過租戶管理程式UI進行設定。如需詳細資訊、請參閱 https://["使用平台服務的考量"^]。

本文件為的補充說明 https://["《英文》《英文》（英文）StorageGRID"^] 並提供搜尋整合服務的端點和庫位組態逐步指示和範例。此處包含的Amazon Web Services（AWS）或內部部署彈性搜尋設定說明僅供基本測試或示範用途。
...
*** 開啟瀏覽器索引標籤 https://["https://checkip.amazonaws.com"^] 尋找您的公有IP。
+
[source, json]
----
{
