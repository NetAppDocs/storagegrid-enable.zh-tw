...
.步驟
. 如果新位置將使用新的 Grid 網路子網路，link:https://docs.netapp.com/us-en/storagegrid/expand/updating-subnets-for-grid-network.html["將子網路新增至 Grid 網路子網路清單"^]
...
. 使用 SSH 連接到應用裝置，然後使用 'shutdown -h now' 關閉節點。
