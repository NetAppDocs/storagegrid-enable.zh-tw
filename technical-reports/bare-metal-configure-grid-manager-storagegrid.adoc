...
.步驟
. <<瀏覽至 Grid Manager>>
. link:bare-metal-storagegrid-license.html["指定StorageGRID 不含授權的資訊"]
. link:bare-metal-add-sites-storagegrid.html["新增站台至 StorageGRID"]
. link:bare-metal-grid-network-subnets-storagegrid.html["指定網格網路子網路"]
. link:bare-metal-approve-grid-nodes-storagegrid.html["核准擱置的網格節點"]
. link:bare-metal-ntp-server-storagegrid.html["指定 NTP 伺服器資訊"]
. link:bare-metal-dns-server-storagegrid.html["指定網域名稱系統伺服器資訊"]
. link:bare-metal-system-passwords-storagegrid.html["指定StorageGRID 「系統密碼」"]
. link:bare-metal-review-config-complete-storagegrid-install.html["檢閱組態並完成安裝"]
