...
.步驟
. 重複該link:veritas-configure-storagegrid-s3-object-lock.html#configure-storagegrid-s3-object-lock-default-bucket-retention["基本組態"]部分中的步驟 1-3，但這次選擇「使用 S3 物件鎖定以 WORM 模式儲存資料」選項。按一下「下一步」。
+
