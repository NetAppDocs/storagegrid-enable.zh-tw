...
----
+
現在您可以使用授權權杖字串透過 cURL 存取指標。存取指標的過程與第 link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["StorageGRID 中的進階監控功能"] 節的步驟類似。但是，為了示範目的，我們展示一個範例，其中在「指標」類別中選擇了 GET /grid/metric-labels/{label}/values。
