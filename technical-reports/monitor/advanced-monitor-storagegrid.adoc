...
image:monitor-metrics-page.png["監控指標頁面"]
...
+
image:monitor-metrics-query.png["監控指標查詢"]

+
回應中包含的資訊與透過 Prometheus URL 查詢所取得的資訊相同。您可以再次看到目前在每個儲存節點上建立的 HTTP 工作階段數目。您也可以下載 JSON 格式的回應、以利閱讀。下圖顯示 Prometheus 查詢回應範例。

+
image:monitor-sample-prometheus-query-responses.png["Monitor-sample-Prometheus-query-Response.s"]
