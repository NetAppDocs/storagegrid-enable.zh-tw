...
NOTE: 表格中的數字是針對每種 StorageGRID 服務類型，而非整個網格或每個實體主機。我們將根據部署選項，在本文件後續部分計算每個實體主機的數字link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["實體主機配置與需求"]。++ ++ 標有星號的路徑或檔案系統將由安裝程式在 StorageGRID 容器中建立。管理員無需手動設定或建立檔案系統，但主機需要區塊裝置來滿足這些要求。換句話說，區塊裝置應透過命令  `lsblk` 顯示，但不得在主機作業系統中格式化或掛載。++ ++
...
|===
在本例中、下表所示的磁碟大小是每種容器類型所需的磁碟大小。每台實體主機的需求說明請參閱本文件後面的 link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["實體主機配置與需求"]。
...
如果您未使用 iSCSI 儲存，則必須確保 host1、host2 和 host3 包含足夠容量的區塊裝置以滿足其需求。請參閱 link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["每種容器類型的磁碟大小"] 以了解 host1、host2 和 host3 的儲存需求。
...
----
. 使用步驟 2 中的啟動器名稱，將儲存裝置上的 LUN （link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["儲存需求"] 表中顯示的數量和大小）對應到每個儲存節點。
