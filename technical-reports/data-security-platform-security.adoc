...
| 增強的 SSH 行為  a| 
安裝前預設禁用 SSH。在預設狀態下，僅在連結本機管理連接埠位址上啟用 SSH 存取。管理者和 root 使用者密碼設定為裝置計算控制器序號。僅允許在序列控制台和圖形控制台（BMC KVM）上登入。任何網路連接埠上的 SSH 均已停用。
| 增強網路存取保護。 | SEC 規則 17a-4(f) CTFC 1.31(c) - （ d ）（ FINRA ）規則 4511(c) 
